...
| Top GSU gLOWCOST tile | SiPM 1 (△) | CH2 | A |
| Bottom GSU gLOWCOST tile | SiPM 2 (★) | CH3 | B |
...
The first attempt set SiPM 1 (△) and SiPM 2 (★) to their measured `Vbr + 3 V` values, but CH0 and CH1 were not yet assigned controlled biases. That run was rejected.

A second attempt used the Hamamatsu typical absolute Vbr of 53 V for the EPIC-tile SiPMs on CH0 and CH1. At 20.82 C this placed both channels near 55.77 V. This was also rejected as the operating-point choice: CH0 saturated the scope and its response was not on the same board-referenced voltage scale as the measured SiPM 1 (△) and SiPM 2 (★) intercepts.

The corrected provisional setting uses the mean measured Vbr of SiPM 1 (△) and SiPM 2 (★), 50.543 V at 20.4 C, as a proxy for each EPIC-tile SiPM. This is not presented as a measurement of their Vbr. At 20.94 C the applied effective biases were:
...
Six events passed these strict cuts in 81.49 s, corresponding to 4.4 events/min. Four of the six had SiPM 1 (△) and SiPM 2 (★) excursions above 200 mV. This sample is too small for an efficiency result, but its rate is compatible with the few-per-minute FPGA CH0-CH1 coincidence scale.

## Production acquisition

The production run was started at 18:11 EDT:

The run is stored in the dated 2026-09-23 experiment directory on the acquisition computer and is identified by its `20260923_181800` start-time suffix.
